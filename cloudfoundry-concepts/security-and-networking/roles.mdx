...
This topic tells you about orgs and spaces in <AppRuntimeFirst/> foundations. It also describes the default permissions for user roles in <AppRuntimeAbbr/>.

<AppRuntimeAbbr/> uses a role-based access control (RBAC) system to grant appropriate permissions to <AppRuntimeAbbr/> users.

Admins, Org Managers, and Space Managers can assign user roles using the Cloud Foundry Command Line Interface (cf CLI). For more information, see [Users and Roles](../cf-cli/getting-started.html#user-roles) in _Getting Started with the cf CLI_<OrAppsMan/>.
...
A user account represents an individual person within the context of a <AppRuntimeAbbr/> foundation. A user can have one or more roles. These roles define the user's permissions in orgs and spaces.
...
The following describes each type of user role in <AppRuntimeAbbr/>:
...
Each user role includes different permissions in a <AppRuntimeAbbr/> foundation. The following sections describe the permissions associated with each user role in both active and suspended orgs in <AppRuntimeAbbr/>.

### <a id='activeroles'></a> Roles and permissions for active orgs

The following table describes the default permissions for various <AppRuntimeAbbr/> roles in active orgs.

{config.platform_code === 'CF' ? (
    <>
        You can use feature flags to edit some of the default permissions in the following table.<br/>
...
{config.platform_code === 'CF' ? (<OSSRolesTable/>) : (<PCFRolesTable/>)}
...
import SuspendedOrgRolesTable from './_suspended_org_roles_table.md';

{config.platform_code === 'CF' ? (<SuspendedOrgRolesTable/>) : (<PCFSuspendedRolesTable/>)}
